...
// ArtDMX Sync packet for jspack
var ArtSyncPacketFormat = "!8sHHBB";
...
class sender {
  ...
  constructor(opt, parent) {
    ...
    if (this.universe > 32767) {
      this.handleError(new Error("Invalid Universe, must be smaller than 32768"));
    }
    if (this.subnet > 15) {
    ...
    if (this.net < 0 || this.subnet < 0 || this.universe < 0) {
      this.handleError(
        new Error("Subnet, Net or Universe must be 0 or bigger!"),
      );
    }
    this.parent.logger.info(
      "new dmxnet sender started with params: %o",
      options,
    );
  ...
  transmit(callback) {
    ...
    if (this.socket_ready) {
      ...
      client.send(udppacket, 0, udppacket.length, this.port, this.ip, (err) => {
        if (err) this.parent.handleError(err);
        this.parent.logger.silly(
          "ArtDMX frame sent to " + this.ip + ":" + this.port,
        );
  ...
  transmitSync() {
    // Only transmit if socket is ready
    if (this.socket_ready) {
      // Build packet: ID Int8[8], OpCode Int16 0x5200 (conv. to 0x0052),
      // ProtVer Int16, Sequence Int8, PhysicalPort Int8,
      // SubnetUniverseNet Int16, Length Int16
      var udppacket = Buffer.from(
        jspack.Pack(ArtDmxHeaderFormat, [
          "Art-Net",
          0,
          0x0052,
          14,
      ...
      client.send(udppacket, 0, udppacket.length, this.port, this.ip, (err) => {
        if (err) this.parent.handleError(err);
        this.parent.logger.silly(
          "ArtSync frame sent to " + this.ip + ":" + this.port,
        );
      });
    }
  }
  ...
  constructor(opt, parent) {
    ...
    if (this.net > 127) {
      this.parent.handleError(
        new Error("Invalid Net, must be smaller than 128"),
      );
    }
    if (this.universe > 31) {
      this.parent.handleError(
        new Error("Invalid Universe, must be smaller than 32"),
      );
    }
    if (this.subnet > 15) {
      this.parent.handleError(
        new Error("Invalid subnet, must be smaller than 16"),
      );
    }
    if (this.net < 0 || this.subnet < 0 || this.universe < 0) {
      this.parent.handleError(
    ...
    this.parent.logger.info(
      "new dmxnet receiver started with params %o",
      options,
    );
    // init dmx-value array
    this.values = [];
...
var dataParser = function (msg, rinfo, parent) {
  ...
  if (rinfo.size < 10) {
    parent.logger.silly("Payload too short");
    return;
  }
  ...
  switch (opcode) {
    case 0x5000:
      ...
      parent.logger.debug(
        "Received frame for SubUniNet 0x" + universe.toString(16),
      );
